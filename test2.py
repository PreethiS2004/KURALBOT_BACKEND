...
def fetch_field_from_mongodb(field, limit=100):
    client = pymongo.MongoClient("mongodb://127.0.0.1:27017/")
    db = client['CHATBOT']
    collection = db['DETAIL2']
    
    # Fetch documents that contain the specified field and the 'number' field, with a limit on how many to fetch at once
    docs = collection.find({field: {"$exists": True}, 'number': {"$exists": True}}, {field: 1, 'number': 1}).limit(limit)
    ...
    if not field_data:
        ...
        logging.info(f"Fetched {len(field_data)} documents with field: {field}")
...
def process_input(user_text, field):
    ...
    embeddings_user_text = generate_embeddings_in_batches([user_text])
    ...
    # Generate embeddings for MongoDB field data in batches
    field_texts = [preprocess_text(text) for text in field_data.values()]
    embeddings_field_data = generate_embeddings_in_batches(field_texts)
